...
def NormaliseData(dataframe):
    ...
    dataframe_normalized = pd.DataFrame(array_normalised, columns=dataframe_to_normalize.columns, index=dataframe.index)
...
def cleanupPlayerPositions(dataframe):            
    dataframe['player_positions_one'] = dataframe['player_positions'].map(lambda x: x.split(',')[0])
    ...
    dataframe.drop(columns =['club_name', 'league_rank','release_clause_eur'], inplace=True) 
...
def SplitDataframe(dataframe):
    ...
    goalkeeping_stats_to_remove = ["shooting", "pace", "passing", "dribbling", "defending", "physic", "skill_moves"]
    goalkeepers_dataframe.drop(columns=goalkeeping_stats_to_remove, inplace=True, errors='ignore')
    
    # For outfield players, drop any goalkeeping stat related columns
    outfield_related_stats_to_remove = ["gk_speed", "gk_diving", "gk_handling", "gk_kicking", "gk_positioning", "gk_reflexes"]
    outfield_dataframe = dataframe[dataframe['player_positions_one'] != 1]
    outfield_dataframe.drop(columns=outfield_related_stats_to_remove, inplace=True, errors='ignore')
