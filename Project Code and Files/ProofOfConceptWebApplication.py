...
dataframe_normalised = pd.DataFrame(
    array_normalised, columns=new_dataframe.columns, index=new_dataframe.index
)
joined_dataframe = pd.concat(
    [dataframe[columns_not_being_normalised], dataframe_normalised], axis=1
)
joined_dataframe["player_positions_one"] = joined_dataframe["player_positions"].map(
    lambda x: x.split(",")[0]
)
...
def linearRegression(playerName):
    ...
    regressionModel = regr.predict(
        rowOfPlayer.drop(rowOfPlayer.columns[[0, 3]], axis=1)
    )
...
def randomForestRegression(playerName):
    ...
    maxFeatureVal = st.selectbox(
        "What max feature value would you like to use?", ("log2", "auto", "sqrt")
    )
